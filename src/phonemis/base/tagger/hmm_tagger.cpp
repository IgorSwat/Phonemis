...
using conversions::u32_to_utf8;

HMMTagger::HMMTagger(const std::string& hmm_data_path) {
	// Load the input JSON file
	nlohmann::json json_obj = utils::io::load_json(hmm_data_path);

	// Validate input JSON file structure by checking the required top-level fields
  ...
  // We can simultaneously load all the possible tags here without an external 
  // config field or file, since all the tags must appear in start_prob field of the JSON file.
...
void HMMTagger::tagSentence(std::span<Token> sentence) const {
  ...
  // Calculates probabilities for the first word in the sentence.
    ...
    double emit_p = emission_probs_.at(tag).contains(u32_to_utf8(sentence[0].text))
                    ? emission_probs_.at(tag).at(u32_to_utf8(sentence[0].text)) : EPSILON;
		...
		if (unicode::isalpha(sentence[0].text[0])) {
			std::u32string lowerized = sentence[0].text;
			lowerized[0] = unicode::tolower(lowerized[0]);
			auto lowerized_utf8 = u32_to_utf8(lowerized);

			emit_p = emission_probs_.at(tag).contains(lowerized_utf8)
                ? emission_probs_.at(tag).at(lowerized_utf8) : EPSILON;
  ...
  // Processes through the rest of the sentence.
    ...
    const auto& word = sentence[t].text;
		auto word_utf8 = u32_to_utf8(word);
      ...
      double emit_p = emission_probs_.at(curr_tag).contains(word_utf8)
                    ? emission_probs_.at(curr_tag).at(word_utf8) : EPSILON;
